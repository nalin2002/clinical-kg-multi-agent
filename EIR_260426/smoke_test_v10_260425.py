...
Flags this script consumes (everything else passes through to v9.main()):
    ...
    --edge-model MODEL      Override Stage 2 (edge / relation reasoning)
                            ...
                            Same allowed-model list as --node-model.
...
if _kb_override:
    ...
    print(f"[v10] CURATED_KB overridden via env: {v8.CURATED_KB}", flush=True)

# Separate output dir so v9 and v10 runs don't mix.
v9.OUT_DIR = v8.EIR_ROOT / "eir_results" / "smoke_test_glm47_specialized_parallel_v10"
...
# extraction run dir.
SCORING_BASE: Path = EIR_ROOT / "eir_results" / "v10_for_organizer"
...
def _run(cmd: list, cwd: Path, env_extra: dict | None = None) -> None:
    ...
    print(f"\n[v10] $ {pretty}", flush=True)
    print(f"[v10]   (cwd: {cwd})", flush=True)
    result = subprocess.run(cmd, cwd=str(cwd), env=env)
    if result.returncode != 0:
        sys.exit(f"\n[v10] FAILED at: {pretty}\n"
                 f"[v10]   exit code: {result.returncode}")
...
def _score_run(run_dir: Path, score_name: str | None,
    ...
    print("\n" + "=" * 72, flush=True)
    print(f"[v10] POST-BATCH SCORING CHAIN", flush=True)
    ...
    if not CONVERT_SCRIPT.exists():
        sys.exit(f"[v10] ERROR: convert script not found at {CONVERT_SCRIPT}")
...
def main() -> None:
    ...
    if node_model:
        ...
        # …AND patch the captured default in OpenRouterClient.__init__.
        # Default arg values are evaluated once at function-def time, so
        # `def __init__(self, api_key, model=OPENROUTER_MODEL, ...)` captured
        # the original value before our import-time patch could land. v9's
        # InstrumentedOpenRouterClient calls super().__init__(api_key)
        # without an explicit `model=`, so it would pick up the stale default.
        # Updating __init__.__defaults__ in-place fixes it for every client
        # constructed afterwards (both v8.OpenRouterClient and the v9 subclass
        # via super() chain).
        import inspect
        ...
        print(f"[v10] Stage 1 (entities) model: {node_model}", flush=True)
    else:
        print(f"[v10] Stage 1 (entities) model: {v8.OPENROUTER_MODEL} (default)", flush=True)
    if edge_model:
        v8.MODEL_OVERRIDES_BY_CATEGORY = {
            c: edge_model for c in v8.MODEL_OVERRIDES_BY_CATEGORY
        }
        print(f"[v10] Stage 2 (edges) model: {edge_model}", flush=True)
    else:
        sample_edge_model = next(iter(v8.MODEL_OVERRIDES_BY_CATEGORY.values()), "(none)")
        print(f"[v10] Stage 2 (edges) model: {sample_edge_model} (default)", flush=True)
    ...
    print("[v10] prompts swapped to prompts_v10_260425", flush=True)
    ...
    for _cat in v8.ACTIVE:
        v8.ACTIVE[_cat] = True
    print("[v10] ACTIVE: all 13 categories ON by default "
    ...
    print(f"[v10] extraction output dir: {v9.OUT_DIR}", flush=True)
    if no_score:
        print("[v10] --no-score: extraction only, no scoring chain",
              flush=True)
    else:
        print(f"[v10] scoring chain enabled — results land under "
    ...
    print(f"\n[v10] extraction wall-clock: {extraction_ms/1000:.1f}s",
          flush=True)
    ...
    if run_dir is None:
        print("[v10] WARNING: could not locate a new run dir under "
